...
//       // this.notificacion.mostrarError('Formulario inválido');
//       console.error('Formulario inválido', this.form.errors);
//       return;
//     }

//     const calendario: CalendarioCreacionDTO = {
//       id_usuario: 17,
...
//     const actividades: ActividadCreacionDTO[] = this.form.value.actividades?.map((act: any) => ({
//       titulo: act.titulo,
//       estado: act.estado,
...
//     this.enviarDatos(calendario, actividades);
//   }
...
//       next: (res: CalendarioRespuestaCreacionAPI) => {
//         const idCalendario = res.data?.id_calendario;

//         if (idCalendario) {
//           this.notificacion.mostrarExito('Calendario creado correctamente');
//           this.posteoFormulario.emit(calendario);
//           this.router.navigate(['/']);

//           actividades.forEach((actividad) => {
//             actividad.id_calendario = idCalendario;
//             this.actividadService.crearActividad(actividad).subscribe({
//               next: (resAct) => {
//                 const idActividad = resAct.data?.id_actividad;
//                 actividad.subactividades?.forEach((sub) => {
//                   sub.id_actividad = idActividad;
//                   this.subactividadService.crearSubactividad(sub).subscribe({
//                     next: () => {},
//                     error: () => this.notificacion.mostrarError('Error al crear una subactividad')
//                   });
//                 });
//               },
//               error: () => this.notificacion.mostrarError('Error al crear una actividad')
//             });
//           });
//         } else {
//           this.notificacion.mostrarError('No se obtuvo el ID del calendario');
//         }
//       },
...
export class FormularioCalendariosComponent implements OnInit {
  ...
  guardar(): void {
    ...
    // Crear calendario con actividades y subactividades
    this.calendariosService.crearCalendario(calendario).subscribe({
      next: (res: CalendarioRespuestaCreacionAPI) => {
        const idCalendario = res.data?.id_calendario;
