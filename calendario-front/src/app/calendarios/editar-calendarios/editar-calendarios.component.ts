...
export class EditarCalendariosComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.calendariosService.consultarCalendario(id).subscribe({
      next: (respuesta) => {
        // Manejo de error cuando backend responde status=0
        if (respuesta.status === 0) {
          this.notificacionService.mostrarError(respuesta.message || 'Error al cargar el calendario');
      ...
      error: (error) => {
        if (error.status === 403) {
          this.notificacionService.mostrarError('No tienes permiso para editar este calendario.');
          this.router.navigate(['/dashboard']);
        } else if (error.status === 404) {
          this.notificacionService.mostrarError('Calendario no encontrado.');
          this.router.navigate(['/dashboard']);
        } else {
          this.notificacionService.mostrarError('Error al cargar el calendario');
